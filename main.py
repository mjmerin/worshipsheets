...
async def scrape_lyrics(
    ...
    # Process each URL (similar to your original main script)
    for url in urls:
        url = url.strip()
        if not url:
            continue
...
@app.post("/api/scrape")
async def api_scrape_lyrics(request: URLRequest):
    """
    API endpoint for programmatic access
    """
    return await scrape_lyrics(request.urls)
...
# Cleanup function (in production, use a background task)
